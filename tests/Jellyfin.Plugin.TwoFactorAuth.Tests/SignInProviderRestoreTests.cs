...
/// and lists them so the first start after a reinstall takes them back.
/// </summary>
public sealed class SignInProviderRestoreTests : IDisposable
{
    ...
    public async Task A_provider_id_in_another_case_is_still_this_plugins()
    {
        ...
        var handedBack = await SignInProviderRestore.RestoreAsync(
            new[] { shouted }, Target, _ => Task.CompletedTask, NullLogger.Instance);
    ...
    [Fact]
    public void Uninstall_keeps_the_accounts_an_earlier_uninstall_listed()
    {
        // A reinstall whose save failed leaves an entry to retry; a second
        // uninstall before that retry must not drop it.
    ...
    [Fact]
    public async Task Reinstall_takes_back_only_the_accounts_still_where_the_uninstall_left_them()
    {
    ...
    public async Task A_failed_lookup_keeps_the_list_for_the_next_start()
    {
        ...
        await SignInProviderRestore.ReclaimAfterReinstallAsync(userManager, _paths, NullLogger.Instance);
