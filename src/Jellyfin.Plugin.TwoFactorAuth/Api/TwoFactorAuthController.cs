...
public class TwoFactorAuthController : ControllerBase
{
    ...
    // -------------------------------------------------------------------------
    // v2.4: GET /TwoFactorAuth/MySessions [Authorize] — caller's active
    // sessions across all devices. User-facing equivalent of the admin
    // session list. Powers the "Where am I signed in?" Setup-page section.
    // -------------------------------------------------------------------------

    public class MySessionEntry
    {
    ...
    [HttpGet("MySessions")]
    [Authorize]
    [ProducesResponseType(typeof(IReadOnlyList<MySessionEntry>), StatusCodes.Status200OK)]
    public ActionResult<IReadOnlyList<MySessionEntry>> GetMySessions()
    {
        var userId = GetCurrentUserId();
        var currentDeviceId = Request.Headers["X-Emby-Device-Id"].FirstOrDefault()
            ?? Request.Headers["X-Emby-DeviceId"].FirstOrDefault()
            ?? string.Empty;

        // Field names mirror what setup.html's renderSessions() reads —
        // appName / appVersion / lastActivity / nowPlaying — so the existing
        // JS renderer Just Works. ASP.NET serializes property names as
        // camelCase by default.
        var sessions = _sessionManager.Sessions
            .Where(s => s.UserId == userId)
            .OrderByDescending(s => s.LastActivityDate)
            .Select(s => new MySessionEntry
            {
                Id = s.Id,
                AppName = s.Client ?? string.Empty,
                AppVersion = s.ApplicationVersion ?? string.Empty,
                DeviceName = s.DeviceName ?? string.Empty,
                DeviceId = s.DeviceId ?? string.Empty,
                RemoteEndPoint = s.RemoteEndPoint ?? string.Empty,
                LastActivity = s.LastActivityDate,
                IsActive = s.IsActive,
                IsCurrent = !string.IsNullOrEmpty(currentDeviceId)
                    && string.Equals(s.DeviceId, currentDeviceId, StringComparison.Ordinal),
                NowPlaying = s.NowPlayingItem?.Name ?? string.Empty,
            })
            .ToList();
    ...
    // -------------------------------------------------------------------------
    // v2.4: POST /TwoFactorAuth/MySessions/{id}/Revoke [Authorize] — revoke
    // one of the caller's own sessions. Returns 404 for both "not found" and
    // "not yours" so a user can't probe other users' session ids by guessing.
    // -------------------------------------------------------------------------
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public ActionResult RevokeMySessionByOwner([FromRoute] string id)
    {
        var userId = GetCurrentUserId();
        ...
        _logger.LogInformation("[2FA] User self-revoked session {SessionId}", id);
        return NoContent();
    }
